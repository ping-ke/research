...
style: |
  ...
  section.lead h1 { font-size: 52px; color: #ffffff; }
  section.lead h2 { font-size: 32px; color: #ccccee; }
  h1 { font-size: 44px; color: #1a1a2e; }
  h2 { font-size: 36px; color: #2a2d4a; margin-bottom: 36px; }
  table { font-size: 22px; width: 100%; }
  code { font-size: 22px; line-height: 1.8; }
  pre { font-size: 22px; line-height: 1.8; }
  blockquote { border-left: 4px solid #e94560; font-style: italic; }
  img { display: block; margin: 0 auto; max-width: 100%; max-height: 80vh; object-fit: contain; }
---
...
> "现代版 Folding@home"
> — NVIDIA CEO 黄仁勋，评价 Covenant-72B（SN3 Templar）

- **去中心化训练**：70+ 无许可节点，无数据中心，仅靠普通商用互联网完成 **72B** 模型预训练
- **MMLU 67.1**：对标 Llama-2-70B，去中心化训练首次达到主流开源模型水准
- **市场验证**：SN3 alpha 一月涨 **444%**，TAO 峰值 **$377**
...
![h:560](./images/how-bittensor-actually-work.png)
...
**资金流**
![w:1280](images/mermaid-fund-flow.png)
...
![h:560](images/mermaid-miner-validator.png)

---

## 每个 block 产出的 0.5 TAO 如何在子网间分配？
```
...
4. 裁剪 + 归一化：
   z_i = max(S_i - L, 0)          把最差子网的 z_i 归零，其余子网按相对优势分配 emission
...
![h:560](images/mermaid-dtao-emission.png)
...
![w:1280](images/mermaid-subnet-lifecycle.png)

- 净流入多 → 更多 emission;　净流出 → emission 归零
- 拥有 subnet 方式：创建，购买
- subnet 总量：256，大于时这个数将会末位淘汰
...
Step 2  Consensus    C[j] = weighted_median(W[:,j], S)  ← 加权中位数，抵抗极端评分
...
![h:560](images/mermaid-consensus-params.png)
...
![h:560](images/mermaid-call-chain.png)
...
>  Sample Interface: github.com/latent-to/bittensor-subnet-template
