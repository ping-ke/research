...
sequenceDiagram
    ...
    participant AMM as Subnet AMM Pool
    ...
    S->>AMM: 质押 100 TAO
    AMM->>AMM: x * y = k，计算汇率
    AMM-->>S: 返回 Alpha Token

    Note over Sub: 每个区块 (~12s)
    Sub->>Sub: 产出 1 TAO emission
    Sub->>AMM: 按 alpha token 相对价格分配 emission
    AMM->>AMM: TAO 储备增加 → alpha 价格上升

    Sub->>M: Miner incentive（按 Yuma Consensus 分数）
    Sub->>V: Validator dividend（按 bond）

    S->>AMM: 取消质押（归还 Alpha Token）
    AMM-->>S: 返回 TAO（含增值或亏损，存在滑点）
```
...
flowchart LR
    ...
    G -->|alpha 价格下降<br/>emission 减少| H[Subnet 萎缩/退出]
```
...
> 演讲时查看 SN3 当前活跃 Miner：[taostats.io/subnets/netuid-3](https://taostats.io/subnets/netuid-3) → Metagraph 标签页
>
> 或用命令行：
> ```bash
...
> 演讲时打开 [taostats.io/subnets/netuid-3](https://taostats.io/subnets/netuid-3) → Metagraph 标签页，对照下表讲解每列含义
>
...
| **Stake** | Stake Weight | 所有 | 质押权重 (α + τ × 0.18)，决定共识影响力和 emission | ≥ 0 TAO |
...
- **Alpha_Stake (α)**：通过子网 AMM 池获得的 alpha token 质押
- **TAO_Stake (τ)**：直接质押的 TAO，按 0.18 系数折算
...
- **Taostats**: [taostats.io/subnets/netuid-3](https://taostats.io/subnets/netuid-3)
...
1. 每个区块 (~12s) 产出 **1 TAO**（首次 halving 前，2025.12 halving 后为 0.5）
2. 总 emission 在子网间分配（dTAO 下按 alpha token 相对市价比例分配）
3. 子网内部，Yuma Consensus 运行：
   - Validator 权重向量聚合（质押加权中位数）
   - 计算 Miner **incentive 分数**
   - 计算 Validator **dividend 分数**（基于 bonds）
   - Emission 分配：Miner 获 incentive，Validator 获 dividend，Subnet Owner 获取其份额

### 以 SN3 为例

> 演讲时打开 [taostats.io/subnets/netuid-3](https://taostats.io/subnets/netuid-3) 的 Distribution 标签页展示
...
| | Pre-dTAO | Post-dTAO |
|--|---------|-----------|
| **Emission 分配** | Root Network 64 个 Validator 投票决定 | 市场价格信号驱动 |
| **子网估值** | 少数人控制 | 全市场参与价格发现 |
| **质押方式** | 委托给 Root Validator | 直接质押到子网 AMM 池 |
...
```
质押流程:
TAO → 进入子网 AMM 池 → 获得 Alpha Token（按当前汇率）

Emission 流向:
新 TAO emission → 流入子网池 → 增加池中 TAO 储备 → 推高 Alpha Token 价格

取消质押:
Alpha Token → 归还给池 → 获得 TAO（按当前汇率，存在滑点）
```
...
| 5-9 min | **架构与角色** | Subtensor 链 + Subnet + Miner/Validator。用 `btcli subnet metagraph --netuid 3` 或 [taostats SN3 Metagraph](https://taostats.io/subnets/netuid-3) 直接展示当前 SN3 的真实 Miner 列表 | 幻灯片 + **Live 命令/浏览器** |
...
- 打开 [taostats.io/subnets/netuid-3](https://taostats.io/subnets/netuid-3)
...
| Miner 是什么 | [taostats.io/subnets/netuid-3](https://taostats.io/subnets/netuid-3) → Metagraph | "SN3 当前有这些活跃节点" |
...
| 项目 | 提前准备 | 耗时 |
|------|---------|------|
| Python 环境 + `pip install bittensor` | 必须 | 5 min |
...
- [SN3 Metagraph](https://taostats.io/subnets/netuid-3)
